...
var FrmEmissaoNFNew : TFrmEmissaoNFNew;
...
procedure TFrmEmissaoNFNew.FormClose(Sender: TObject; var Action: TCloseAction);
begin
stsNotaE := '';
...
dmBaseDados.tblANotaFiscalNew.IndexName   := '';
...
procedure TFrmEmissaoNFNew.mskNrNotaFiscalExit(Sender: TObject);
var strNrN : Integer;
    msgConfirma : String;
begin
...
if (dmBaseDados.tblANotaFiscalNew.FindKey([mskNrNotaFiscal.Text])) then
 BEGIN
   ...
   Begin
    ...
    if (Application.MessageBox(pChar(msgConfirma),'C O N F I R M A?',MB_YESNO) = IDYES) then
     begin
      ...
      dmBaseDados.tblANotaFiscalNew.Delete;
     end
    else
     begin
      mskNrNotaFiscal.SetFocus;
...
procedure TFrmEmissaoNFNew.mskNPedidoExit(Sender: TObject);
var str0NPed, strCdCli, msgConfirma, msgDesAc : String;
    fltVTot : Double;
begin
dmBaseDados.tblANotaFiscalNew.IndexName := 'IndPedidoNF';
if( (mskNPedido.Text <> '') )then
  ...
  END;
//---------------------
if (dmBaseDados.tblANotaFiscalNew.Locate('Pedido',mskNPedido.Text,[loCaseInsensitive]) = True) then
  begin
   if (dmBaseDados.tblANotaFiscalNewStatus.AsString = '100') then
    begin
...
procedure TFrmEmissaoNFNew.btnGravaClick(Sender: TObject);
     ...
     WHILE( (not dmBaseDados.tblPedidos.Eof)and(strComp1 = strComp2) )do
       BEGIN
        ...
        if( (dmBaseDados.tblPedidosCST.AsString = '010')or(dmBaseDados.tblPedidosCST.AsString = '210') )then
         ...
         {
            ShowMessage('Total Custo strstcustot: ' +FloatToStr(strSTCustoT));
           ShowMessage('ICMS 12 strSTIcms12 : '+FloatToStr(strSTIcms12));
           ShowMessage('Base strSTBase : '+FloatToStr(strSTBase));
           ShowMessage('ICMS Cliente strSTIcmsCli : '+FloatToStr(strSTIcmsCli));
           ShowMessage('ICMS strSTIcms : '+FloatToStr(strSTIcms));  }
...
// Gravando na tblANotaFiscalNew...:

if (stsNotaE = '1') then
 begin
  dmBaseDados.tblANotaFiscalNew.FindKey([mskNrNotaFiscal.Text]);
  dmBaseDados.tblANotaFiscalNew.Edit;
 end
else
 begin
 ...
 dmBaseDados.tblANotaFiscalNewPedido.AsString         := mskNPedido.Text;
 dmBaseDados.tblANotaFiscalNewNrNF.AsString           := mskNrNotaFiscal.Text;
 dmBaseDados.tblANotaFiscalNewDataEmissao.AsDateTime  := Date;
 dmBaseDados.tblANotaFiscalNewCFOP.AsString           := cmbCfop.Text;
 dmBaseDados.tblANotaFiscalNewCodigoCliente.AsInteger := dmBaseDados.tblClientesCodigoCliente.AsInteger;
 ...
 dmBaseDados.tblANotaFiscalNewContaCtblFornecedor.AsString := ''; //fltContaCtblFornecedor;
 dmBaseDados.tblANotaFiscalNewNrMunicipio.AsFloat     := fltNrMunicipio;
 dmBaseDados.tblANotaFiscalNewInscrEstadual.AsString  := dmBaseDados.tblClientesNumeroRG.AsString;
 dmBaseDados.tblANotaFiscalNewRazaoSocial.AsString    := dmBaseDados.tblClientesNomeCliente.AsString;
 dmBaseDados.tblANotaFiscalNewUf.AsString             := dmBaseDados.tblClientesEstado.AsString;
 ...
 dmBaseDados.tblANotaFiscalNewDesconto.AsFloat        := strDescon;
 dmBaseDados.tblANotaFiscalNewDespesasAC.AsFloat      := strDespAC;
 dmBaseDados.tblANotaFiscalNewValorVista.AsFloat      := fltTotal + strDespAC + strFrete - strDescon;
 dmBaseDados.tblANotaFiscalNewFrete.AsFloat           := strFrete;
 dmBaseDados.tblANotaFiscalNewBaseST.AsFloat          := strSomaBaseST;
 dmBaseDados.tblANotaFiscalNewIcmsST.AsFloat          := strSomaIcmsST;
 dmBaseDados.tblANotaFiscalNewIPI.AsFloat             := strSomaIPI;
 dmBaseDados.tblANotaFiscalNewVTributos.AsFloat       := strTotalTrib;
 dmBaseDados.tblANotaFiscalNewTribFed.AsFloat         := strTribFeder;
 dmBaseDados.tblANotaFiscalNewTribEst.AsFloat         := strTribEstad;
 dmBaseDados.tblANotaFiscalNewTribMun.AsFloat         := strTribMunic;
 dmBaseDados.tblANotaFiscalNewBaseCalculo.AsFloat     := fltBase7+fltBase12+fltBase18+fltBase25+fltBaseR12+fltBaseR18;
 dmBaseDados.tblANotaFiscalNewValorICMS.AsFloat       := fltIcms7+fltIcms12+fltIcms18+fltIcms25+fltIcmsR12+fltIcmsR18;
 if (dmBaseDados.tblClientesTipoCliente.AsString = 'CPF') then
  begin
   dmBaseDados.tblANotaFiscalNewCgc.AsString         := strCPF;
  end
 else
  begin
...
procedure TFrmEmissaoNFNew.btnRetornarClick(Sender: TObject);
begin
if (stsNotaE = '1') then
  Begin
...
procedure TFrmEmissaoNFNew.btnImprimeClick(Sender: TObject);
var strStatus, strPgto, strCodigoProduto, strCodProRef, strDescricao,
    ...
    strPFrete, strPOutro, strPDespAC, strDescUn, fTotIcms,fBaseIcms : Double;
...
//--
dmBaseDados.tblANotaFiscalNew.First;
dmBaseDados.tblANotaFiscalNew.Locate('NrNF',mskNrNotaFiscal.Text,[loCaseInsensitive]);
...
//***-------------------------------------------------------------------------------------------------------------------------------------------------***
  //Geração do arquivo para Nota Fiscal Eletrônica...:
    ...
    SHORTDATEFORMAT  := 'dd/mm/yyyy';
    DecimalSeparator := '.';
    Natureza         := Copy(dmBaseDados.tblANotaFiscalNewCFOP.AsString,7,30);
    ...
    AssignFile(ArquivoNFe, 'F:\NFeAt\'+strNPedido+'.txt');
    Rewrite(ArquivoNFe);

      Writeln(ArquivoNFe, 'NOTAFISCAL|1|');
      Writeln(ArquivoNFe, 'A|3.10|NFe'  +ChaveAcesso); // 3.10
      cDV      := '';
      ...
      if( (Copy(dmBaseDados.tblANotaFiscalNewCFOP.AsString,1,5) = '1.202') )then
       begin
      ...
      if (ckbAVista.Checked = True) then
       begin
        Writeln(ArquivoNFe, 'B|35||'  +Natureza+'|'  +'0'+'|'  +'55'+'|'  +'1'+'|'  +NumeroNF+'|'  +DataEmissao+'|'  +DataSaida+'|'  +strEntrSaida+'|'  +idDest+'|'  +'3534401'+'|'  +'1'+'|'  +'1'+'|'  +cDV+'|'  +'1'+'|'  +finNFe+'|'  +indFinal+'|'  +indPres+'|'  +'3'+'|'  +'3.10|'  +dhCont+'|'  +xJust+'|'); //Versao 3.10
       end;
      if (ckbPrazo.Checked = True) then
       begin
        Writeln(ArquivoNFe, 'B|35||'  +Natureza+'|'  +'1'+'|'  +'55'+'|'  +'1'+'|'  +NumeroNF+'|'  +DataEmissao+'|'  +DataSaida+'|'  +strEntrSaida+'|'  +idDest+'|'  +'3534401'+'|'  +'1'+'|'  +'1'+'|'  +cDV+'|'  +'1'+'|'  +finNFe+'|'  +indFinal+'|'  +indPres+'|'  +'3'+'|'  +'3.10|'  +dhCont+'|'  +xJust+'|'); //Versao 3.10
       end;
      if( (Copy(dmBaseDados.tblANotaFiscalNewCFOP.AsString,1,5) = '1.202') )then
       begin
        Writeln(ArquivoNFe, 'BA02|'  +edtChaveFornec.Text +'|');
       end;
    ...
    with (ACBrNFe1.NotasFiscais.Add.NFe) do
      BEGIN
       ...
       // 4.00 "Novatag" na verdade será excluída a Ide.indPag, pois agora terá o grupo "pag"
       //Ide.indPag   := TpcnIndicadorPagamento(StrToInt(indPag)); // TpcnIndicadorPagamento(dmNFe.qNFeIndPag.AsInteger); //cbFormaPagamento.ItemIndex);
       Ide.modelo   := 55;
      ...
      if (dmBaseDados.tblClientesTipoCliente.AsString = 'CPF') then
       begin
        indIEDest := '9';
        Writeln(ArquivoNFe,'E|'  +ClienteDest+'|'  +indIEDest+'|'  +''+'|'  +'|'  +IM+'|'  +emailDest+'|'); //Versao 3.10
        Writeln(ArquivoNFe,'E03|'+strCPF+'|');
       end
      else
       begin
        if( (dmBaseDados.tblClientesNumeroRG.AsString <> 'ISENTO')and(dmBaseDados.tblClientesNumeroRG.AsString <> 'ISENTA')
         ...
         begin
          indIEDest := '1';
          Writeln(ArquivoNFe,'E|'  +ClienteDest+'|'  +indIEDest+'|'  +IEDest+'|'  +'|'  +IM+'|'  +emailDest+'|'); //Versao 3.10
         end
        else
         begin
          indIEDest := '2';
        ...
        Writeln(ArquivoNFe,'E02|'+strCNPJ+'|');
       end;
      Writeln(ArquivoNFe, 'E05|'  +EnderecoDest+'|'  +NumeroDest+'|'  +ComplDest+'|'  +BairroDest+'|'  +CodigoIBGE+'|'  +CidadeDest+'|'  +EstadoDest+'|'  +CEPDest+'|'  +'1058|BRASIL|'  +TelDest+'|');
      ...
      while (not dmBaseDados.tblLogMensal.Eof) do
        Begin
         ...
         NCMProd      := BuscarETroca(NCMProd,'/','');
         NCMProd      := BuscarETroca(NCMProd,',','');
         CFOPProd     := ( Copy(dmBaseDados.tblANotaFiscalNewCFOP.AsString,1,1) + Copy(dmBaseDados.tblANotaFiscalNewCFOP.AsString,3,3) );
         ...
         if (dmBaseDados.tblLogMensalCodigoProduto.AsString <> '') then
           begin
            Writeln(ArquivoNFe, 'H|' +(IntToStr(Contador))+'|' +'|');
            ...
            AliqST := AliqICMS;
            Writeln(ArquivoNFe, 'I|'  +CodigoProd+'|'  +''+'|'  +DescriProd+'|'  +NCMProd+'|'   +NVE+'|'  +CFOPProd+'|'  +UnidadeProd+'|'  +QtdeProd+'|'  +VUnProd+'|'  +VTotalProd+'|'  +''+'|'  +UnidadeTrib+'|'  +QtdeTrib+'|'  +VTotalTrib+'|'  +vPFrete+'|'  +''+'|'  +vPDesc+'|'  +vPOutro+'|'  +indTot+'|'  +xPed+'|'  +nItemPed+'|'  +nFCI+'|'); //3.10
            ...
            if( (Copy(CSTProd,2,2) = '00')or(Copy(CSTProd,2,2) = '20') )then
             Begin
              {if (indIEDest <> '9') then
               Begin
                Writeln(ArquivoNFe, 'N10c|'  +OrigemProd+'|'  +'101'+'|'  +pCredSN+'|'  +vCredICMS+'|');
                 // 4.00
                Imposto.ICMS.CSOSN       := csosn101;
               ...
               Begin
                Writeln(ArquivoNFe, 'N10d|'  +OrigemProd+'|'  +'102'+'|');
                // 4.00
              ...
              fTotIcms    := fTotIcms  + strCalcIcms;
              fBaseIcms   := fBaseIcms + StrToFloat(VTotalProd);
            ...
            if (Copy(CSTProd,2,2) = '10') then
              begin
               ...
               ModalidadeBC := '4';
               pMVAST       := '';
               AliqST       := '0.00';                 //                   N10e|      Orig      |    CSOSN  |       modBCST    |    pMVAST  |pRedBCST|     vBCST  |  pICMSST   |   vICMSST  |    pCredSN  |   vCredICMSSN |
               Writeln(ArquivoNFe, 'N10e|'  +OrigemProd+'|'  +'201'+'|'  +ModalidadeBC+'|'  +pMVAST+'|'  +''+'|'  +BaseST+'|'  +AliqST+'|'  +IcmsST+'|'  +pCredSN+'|'  +vCredICMS+'|');
            ...
            if( (Copy(CSTProd,2,2) = '60')or(Copy(CSTProd,2,2) = '70') )then
              begin
               ModalidadeBC := '';
               Writeln(ArquivoNFe, 'N10g|'  +OrigemProd+'|'  +'500'+'|'  +ModalidadeBC+'|'  +BaseST+'|'  +IcmsST  +'|');
            ...
            //Destacando IPI...:
            {if( ((CSTProd = '010')or(CSTProd = '210'))and(strCfopMudou2 = '0') )then
              begin
               ModalidadeBC := '0';
               Writeln(ArquivoNFe, 'O|||||999|');
               Writeln(ArquivoNFe, 'O07|50|' +ValorIPI+'|');
               Writeln(ArquivoNFe, 'O10|' +VTotalProd+'|' +AliqIPI+'|');
              end;
            if( (strCfopMudou2 = '1') and (rdgTipoOperacao.ItemIndex = 1) )then
              begin
               ModalidadeBC := '0';
               Writeln(ArquivoNFe, 'O|||||999|');
               Writeln(ArquivoNFe, 'O07|50|' +ValorIPI+'|');
            ...
            // PIS
            Writeln(ArquivoNFe, 'Q'  +'|');
            Writeln(ArquivoNFe, 'Q04|07'  +'|');
            ...
            // COFINS
            Writeln(ArquivoNFe, 'S'  +'|');
            Writeln(ArquivoNFe, 'S04|07'  +'|');
      ...
      vBCST   := '0.00'; //FormatFloat('0.00',dmBaseDados.tblANotaFiscalNewBaseST.AsFloat);
      vST     := '0.00'; //FormatFloat('0.00',dmBaseDados.tblANotaFiscalNewIcmsST.AsFloat);
      ...
      vSeg    := '0.00';
      vDesc   := FormatFloat('0.00', dmBaseDados.tblANotaFiscalNewDesconto.AsFloat);
      vII     := '0.00';
      vPIS    := '0.00';
      vCOFINS := '0.00';                                                                         //vOutro  := FormatFloat('0.00',strDespAC + strAcres);
      vOutro  := FormatFloat('0.00',strDespAC);
      vTTotTrib := FormatFloat('0.00',dmBaseDados.tblANotaFiscalNewVTributos.AsFloat);              //vNF     := FormatFloat('0.00',dmBaseDados.tblANotaFiscalNewValorTNF.AsFloat - strDescon + strFrete + strDespAC + strAcres);
      vTribFed  := FormatFloat('R$ 0.00',dmBaseDados.tblANotaFiscalNewTribFed.AsFloat);
      vTribEst  := FormatFloat('R$ 0.00',dmBaseDados.tblANotaFiscalNewTribEst.AsFloat);
      vTribMun  := FormatFloat('R$ 0.00',dmBaseDados.tblANotaFiscalNewTribMun.AsFloat);
      vNF     := FormatFloat('0.00',dmBaseDados.tblANotaFiscalNewValorTNF.AsFloat - strDescon + strFrete + strDespAC);


      {if (strTotalST = 0) then
        begin
         vNF  := FormatFloat('0.00',dmBaseDados.tblANotaFiscalNewValorTNF.AsFloat);
        end
      else
        begin
         vNF  := FormatFloat('0.00',dmBaseDados.tblANotaFiscalNewValorTNF.AsFloat + dmBaseDados.tblANotaFiscalNewIcmsST.AsFloat + dmBaseDados.tblANotaFiscalNewIPI.AsFloat);
        end; }
      Writeln(ArquivoNFe, 'W'  +'|');
      strMsgTrib := 'Valor Aprox. Tributos R$ ' + vTTotTrib +' ('+ FormatFloat('0.00',(dmBaseDados.tblANotaFiscalNewVTributos.AsFloat/dmBaseDados.tblANotaFiscalNewValorTNF.AsFloat)*100) +'%)(Federal: ' + vTribFed +' / Estadual: ' +vTribEst  +' / Municipal: ' +vTribMun +') Fonte: IBPT.  ';
      Writeln(ArquivoNFe, 'W02|'  +vBC+ '|'  +vICMS+ '|'  +vICMSDeson+ '|'  +vBCST+ '|'  +vST+ '|'  +vProd+ '|'  +vFrete+ '|'  +vSeg+ '|'  +vDesc+ '|'  +vII+ '|'  +vIPI+ '|'  +vPIS+ '|'  +vCOFINS+ '|'  +vOutro+ '|'  +vNF+ '|'  +vTTotTrib+ '|'); //Versao 3.10
      ...
      if( (EstadoDest <> 'SP')and((TipoCli = 'CPF')or(IsentoCli = 'ISENTO')) )then
       Begin
        ...
        Writeln(ArquivoNFe, 'W04c|' +vFCPUFDest+    '|');
        Writeln(ArquivoNFe, 'W04e|' +vICMSUFDest+   '|');
        Writeln(ArquivoNFe, 'W04g|' +vICMSUFRemet+  '|');

        dmBaseDados.tblANotaFiscalNew.Edit;
        dmBaseDados.tblANotaFiscalNewFCP.AsFloat        := fltSomaFCP;
        dmBaseDados.tblANotaFiscalNewPartilhaDS.AsFloat := fltSomaPrDS;
        dmBaseDados.tblANotaFiscalNewPartilhaOR.AsFloat := fltSomaPrOR;
        dmBaseDados.tblANotaFiscalNew.Post;
       End;
      ...
      if (rdgFrete.ItemIndex = 0) then
       begin
        Writeln(ArquivoNFe, 'X|0' +'|');
        {if (rdgMesmo.Checked = True) then
         begin
          TransNome     := 'NOME DA TRANSPORTADORA';
          TransIE       := 'INSC. ESTADUAL';
          TransEndereco := 'ENDEREÇO';
          TransUF       := 'ESTADO';
          TransCidade   := 'CIDADE';
          TransCNPJ     := 'CNPJ';
          Writeln(ArquivoNFe, 'X03|'  +TransNome+'|' +TransIE+'|' +TransEndereco+'|' +TransUF+'|' +TransCidade+'|');
          Writeln(ArquivoNFe, 'X04|'  +TransCNPJ+'|');
          Writeln(ArquivoNFe, 'X18|'  +TransPlaca+'|' +TransUFPlaca+'|' +'|');
         end;}
       end;
      if (rdgFrete.ItemIndex = 1) then
       begin
        Writeln(ArquivoNFe, 'X|1' +'|');
       end;
      if (rdgTrans.Checked = True) then
       begin
        Writeln(ArquivoNFe, 'X03|'  +TransNome+'|' +TransIE+'|' +TransEndereco+'|' +TransCidade+'|' +TransUF+'|');
        Writeln(ArquivoNFe, 'X04|'  +TransCNPJ+'|');
        Writeln(ArquivoNFe, 'X18|'  +TransPlaca+'|' +TransUFPlaca+'|' +'|');
       end;
      ...
      if (PesoB <> '0.000') then
       begin
        ...
        Writeln(ArquivoNFe, 'X26|' +Volume+'|'  +Especie+'|'  +''+'|'  +''+'|'  +PesoL+'|'  +PesoB+'|');
      ...
      if( (edtVolume.Text <> '')and(PesoB = '0.000') )then
       begin
        Especie := edtEspecie.Text;
        Writeln(ArquivoNFe, 'X26|' +Volume+'|'  +Especie+'|'  +''+'|'  +''+'|'  +''+'|'  +''+'|');
      ...
      if (strDoc1 <> '') then
       Begin
        ...
        if (strVenc1 <> '') then
         begin
          Writeln(ArquivoNFe, 'Y07|'  +strDoc1+ '|' +strVenc1+ '|' +strValor1+ '|');
        ...
        if (strVenc2 <> '') then
         begin
          Writeln(ArquivoNFe, 'Y07|'  +strDoc2+ '|' +strVenc2+ '|' +strValor2+ '|');
        ...
        if (strVenc3 <> '') then
         begin
          Writeln(ArquivoNFe, 'Y07|'  +strDoc3+ '|' +strVenc3+ '|' +strValor3+ '|');
        ...
        if (strVenc4 <> '') then
         begin
          Writeln(ArquivoNFe, 'Y07|'  +strDoc4+ '|' +strVenc4+ '|' +strValor4+ '|');
        ...
        if (strVenc5 <> '') then
         begin
          Writeln(ArquivoNFe, 'Y07|'  +strDoc5+ '|' +strVenc5+ '|' +strValor5+ '|');
        ...
        if (strVenc6 <> '') then
         begin
          Writeln(ArquivoNFe, 'Y07|'  +strDoc6+ '|' +strVenc6+ '|' +strValor6+ '|');
        ...
        if (strVenc7 <> '') then
         begin
          Writeln(ArquivoNFe, 'Y07|'  +strDoc7+ '|' +strVenc7+ '|' +strValor7+ '|');
        ...
        if (strVenc8 <> '') then
         begin
          Writeln(ArquivoNFe, 'Y07|'  +strDoc8+ '|' +strVenc8+ '|' +strValor8+ '|');
      ...
      if (dmBaseDados.tblANotaFiscalNewValorICMS.AsFloat > 0) then
       begin
        vICMS := FormatFloat('0.00',dmBaseDados.tblANotaFiscalNewValorICMS.AsFloat);
        Writeln(ArquivoNFe, 'Z|' +'|' +edtObsComp.Text+ ' / De acordo com a Portaria Cat 162/2008 artigo 18 Inciso I Alinea b, e permitido o cancelamento de NF-e somente em ate 24 horas apos sua emissao. / ' + strMsgTrib +'|');

         // 4.00
         with (InfAdic) do
          begin
           infCpl     := edtObsComp.Text+ ' / De acordo com a Portaria Cat 162/2008 artigo 18 Inciso I Alinea b, e permitido o cancelamento de NF-e somente em ate 24 horas apos sua emissao. / '  + strMsgTrib;
           infAdFisco := '';
          end;
...
dmBaseDados.tblANotaFiscalNewCaminho2.AsString     := ACBrNFe1.NotasFiscais.Items[0].NomeArq;
dmBaseDados.tblANotaFiscalNewLinhaConting.AsString := strConting;
dmBaseDados.tblANotaFiscalNew.Post;
...
dmBaseDados.tblANotaFiscalNew.Edit;
dmBaseDados.tblANotaFiscalNewStatus.AsInteger     := ACBrNFe1.NotasFiscais.Items[0].NFe.procNFe.cStat;        //ACBrNFe1.WebServices.Consulta.cStat;
...
dmBaseDados.tblANotaFiscalNewChaveAcesso.AsString := Copy(ACBrNFe1.NotasFiscais.Items[0].NFe.infNFe.ID,4,44); //ACBrNFe1.WebServices.Retorno.ChaveNFe;
dmBaseDados.tblANotaFiscalNew.Post;
...
procedure TFrmEmissaoNFNew.ckbAVistaClick(Sender: TObject);
begin
if (ckbAVista.Checked = True) then
 begin
  ckbPrazo.Checked  := False;
...
procedure TFrmEmissaoNFNew.ckbPrazoClick(Sender: TObject);
begin
if (ckbPrazo.Checked = True) then
 begin
  ckbAVista.Checked  := False;
...
procedure TFrmEmissaoNFNew.mskNPedidoKeyPress(Sender: TObject; var Key: Char);
begin
if (Key = #13) then
 begin
  cmbCfop.SetFocus;
...
procedure TFrmEmissaoNFNew.cmbTransportadoraClick(Sender: TObject);
begin
...
procedure TFrmEmissaoNFNew.rdgTransClick(Sender: TObject);
begin
if (rdgTrans.Checked = True) then
 begin
...
procedure TFrmEmissaoNFNew.rdgMesmoClick(Sender: TObject);
begin
if (rdgMesmo.Checked = True) then
 begin
  lblTransportadora.Visible := False;
...
procedure TFrmEmissaoNFNew.edtPesoBrutoKeyPress(Sender: TObject; var Key: Char);
begin
if not (Key in['0'..'9',Chr(8),DecimalSeparator]) then Key:= #0;
end;

procedure TFrmEmissaoNFNew.edtPesoLiqKeyPress(Sender: TObject; var Key: Char);
begin
if not (Key in['0'..'9',Chr(8),DecimalSeparator]) then Key:= #0;
end;

procedure TFrmEmissaoNFNew.edtDescontoKeyPress(Sender: TObject; var Key: Char);
begin
if not (Key in['0'..'9',Chr(8),DecimalSeparator]) then Key := #0;
end;

procedure TFrmEmissaoNFNew.edtDespAcKeyPress(Sender: TObject; var Key: Char);
begin
if not (Key in['0'..'9',Chr(8),DecimalSeparator]) then Key := #0;
end;

procedure TFrmEmissaoNFNew.cmbCfopClick(Sender: TObject);
begin
if( (Copy(cmbCfop.Text,1,5) = '1.202') )then
 begin
  lblChave.Visible       := False;
  edtChaveFornec.Visible := False;
 end
