...
uses Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
     ...
     pcnNFeRTXT, pcnAuxiliar, IniFiles, ShellAPI, pcnRetConsReciNFe, Math,
     TILabel, TISLABELS, ACBrNFeDANFeRLClass, ACBrMail, ACBrBase, ACBrDFe,
...
var formNFeDevolucao : TformNFeDevolucao;
...
procedure TformNFeDevolucao.FormShow(Sender: TObject);
begin
DecimalSeparator := ',';
...
procedure TformNFeDevolucao.btnImprimeClick(Sender: TObject);
var Natureza, NumeroNF, DataEmissao, strStatus, strPgto,
    ...
    strProdIcmsST, strIPI, strPEDiv, strCalcST : Double;
    ...
    chFormaEmissao, chCodNumerico, ChaveAcesso, strCPF : String;
...
if (not dmBaseDados.tblANotaFiscal.Locate('NrNF',mskNrNotaFiscal.Text,[loCaseInsensitive])and(NrNf = strNrN)) then
  Begin
  ...
  Begin
   if( (strNrN = NrNf)or(strNrN > NrNf) )then
    begin
  ...
  End;
//--
dmBaseDados.tblANotaFiscalPedido.AsString         := 'D' + mskNrNotaFiscal.Text;
dmBaseDados.tblANotaFiscalEspecie.AsString        := 'D';
dmBaseDados.tblANotaFiscalNrNF.AsString           := mskNrNotaFiscal.Text;
dmBaseDados.tblANotaFiscalDataEmissao.AsDateTime  := Date;
dmBaseDados.tblANotaFiscalCFOP.AsString           := edtNatureza.Text;
dmBaseDados.tblANotaFiscalCodigoCliente.AsInteger := dmBaseDados.tblFornecedoresCodigoFornecedor.AsInteger;
...
dmBaseDados.tblANotaFiscalDesconto.AsFloat        := strDescNFe;
dmBaseDados.tblANotaFiscalValorVista.AsFloat      := fltTotal - strDescNFe;
...
dmBaseDados.tblANotaFiscalContaCtblFornecedor.AsString := 'D'; //fltContaCtblFornecedor;
dmBaseDados.tblANotaFiscalNrMunicipio.AsFloat     := fltNrMunicipio;
dmBaseDados.tblANotaFiscalInscrEstadual.AsString  := dmBaseDados.tblFornecedoresInscricao.AsString;
dmBaseDados.tblANotaFiscalRazaoSocial.AsString    := dmBaseDados.tblFornecedoresRazaoSocial.AsString;
dmBaseDados.tblANotaFiscalUf.AsString             := dmBaseDados.tblFornecedoresEstado.AsString;
...
dmBaseDados.tblANotaFiscalBaseST.AsFloat          := StrToFloat(edtBaseST.Text); //strSomaBaseST;
dmBaseDados.tblANotaFiscalIcmsST.AsFloat          := strTotalST;
dmBaseDados.tblANotaFiscalIPI.AsFloat             := strSomaIPI;
dmBaseDados.tblANotaFiscalVTributos.AsFloat       := strTotalTrib;
dmBaseDados.tblANotaFiscalBaseCalculo.AsFloat     := StrToFloat(edtBaseIcms.Text); //fltBase0 + fltBase7  + fltBase12 + fltBase18 + fltBase25  + fltBaseR12 + fltBaseR18;
dmBaseDados.tblANotaFiscalValorICMS.AsFloat       := StrToFloat(edtVIcms.Text);    //fltIcms7 + fltIcms12 + fltIcms18 + fltIcms25 + fltIcmsR12 + fltIcmsR18;
...
//***-------------------------------------------------------------------------------------------------------------------------------------------------***
  //Geração do arquivo para Nota Fiscal Eletrônica...:
    ...
    DecimalSeparator := '.';
    Natureza      := edtNatureza.Text; //Copy(dmBaseDados.tblANotaFiscalCFOP.AsString,7,60);
    NumeroNF      := dmBaseDados.tblANotaFiscalNrNF.AsString;
    strNFe        := dmBaseDados.tblANotaFiscalNrNF.AsString;
    ...
    AssignFile(ArquivoNFe, 'F:\NFe\'+ 'D'+mskNrNotaFiscal.Text+'.txt');
    Rewrite(ArquivoNFe);

      Writeln(ArquivoNFe, 'NOTAFISCAL|1|');
      Writeln(ArquivoNFe, 'A|3.10|NFe'  +ChaveAcesso);
      cDV      := '';
      ...
      if (edtChaveFornec2.Text <> '') then
       begin
        Writeln(ArquivoNFe, 'BA02|'  +edtChaveFornec2.Text +'|');
       end;
      if (edtChaveFornec3.Text <> '') then
       begin
        Writeln(ArquivoNFe, 'BA02|'  +edtChaveFornec3.Text +'|');
       end;
      if (edtChaveFornec4.Text <> '') then
       begin
        Writeln(ArquivoNFe, 'BA02|'  +edtChaveFornec4.Text +'|');
       end;
      if (edtChaveFornec5.Text <> '') then
       begin
        Writeln(ArquivoNFe, 'BA02|'  +edtChaveFornec5.Text +'|');
       end;
      if (edtChaveFornec6.Text <> '') then
       begin
        Writeln(ArquivoNFe, 'BA02|'  +edtChaveFornec6.Text +'|');
       end;
      ...
      BEGIN
       Ide.cNF      := StrToInt(NumeroNF);                       // dmNFe.qNFeCNF.AsInteger; //(leNumeroNFe2.Text); //Caso nÃ£o seja preenchido serÃ¡ gerado um nÃºmero aleatÃ³rio pelo componente
      ...
      Writeln(ArquivoNFe, 'C|EDER VALTER ANTONIO RODRIGUES||492265826117||||'  +CRT+'|');
      Writeln(ArquivoNFe, 'C02|04418512000123|');
      Writeln(ArquivoNFe, 'C05|R QUINZE DE NOVEMBRO|263||JARDIM CIPAVA|3534401|OSASCO|SP|06065110|1058|BRASIL||');


      // 4.00 Dados Emitente
      Emit.CNPJCPF           := '04418512000123';
      Emit.IE                := '492265826117';
      Emit.xNome             := 'EDER VALTER ANTONIO RODRIGUES';
      Emit.xFant             := '';

      Emit.EnderEmit.fone    := '';
      Emit.EnderEmit.CEP     := 06065110;
      Emit.EnderEmit.xLgr    := 'R QUINZE DE NOVEMBRO';
      Emit.EnderEmit.nro     := '263';
      Emit.EnderEmit.xCpl    := '';
      Emit.EnderEmit.xBairro := 'JARDIM CIPAVA';
      ...
      while (not dmBaseDados.tblLogMensal.Eof) do
        begin
         ...
         NCMProd      := BuscarETroca(NCMProd,'/','');
         NCMProd      := BuscarETroca(NCMProd,',','');
         CFOPProd     := dmBaseDados.tblLogMensalDocumento.AsString; //( Copy(dmBaseDados.tblANotaFiscalCFOP.AsString,1,1) + Copy(dmBaseDados.tblANotaFiscalCFOP.AsString,3,3) );
         ...
         if (dmBaseDados.tblLogMensalCodigoProduto.AsString <> '') then
           Begin
            ...
            if (Copy(CSTProd,2,2) = '90') then  // Outros
              begin              // N10h|      orig      |    CSOSN  |    modBC         |      vBC     |    pRedBC  |      pICMS   |      vICMS    |     modBCST |    pMVAST  |    pRedBCST  |    vBCST  |    pICMSST  |    vICMSST  |    pCredSN  |    vCredICMSSN  |
               Writeln(ArquivoNFe, 'N10h|'  +OrigemProd+'|'  +'900'+'|'  +ModalidadeBC+'|'  +BaseICMS+'|'  +pRedBC+'|'  +AliqICMS+'|'  +ValorICMS+'|'  +modBCST+'|'  +pMVAST+'|'  +pRedBCST+'|'  +vBCST+'|'  +pICMSST+'|'  +vICMSST+'|'  +pCredSN+'|'  +vCredICMSSN+'|');
      ...
      dmBaseDados.tblANotaFiscal.Edit;
      dmBaseDados.tblANotaFiscalPIS.AsFloat    := strTotalPIS;
      dmBaseDados.tblANotaFiscalCOFINS.AsFloat := strTotalCOFINS;
      dmBaseDados.tblANotaFiscalValorVista.AsFloat := (dmBaseDados.tblANotaFiscalValorVista.AsFloat)-(strDescRamo6 + strDescRamo7);
      dmBaseDados.tblANotaFiscal.Post;
      dmBaseDados.tblANotaFiscal.FlushBuffers;
      vBC     := FormatFloat('0.00',dmBaseDados.tblANotaFiscalBaseCalculo.AsFloat);
      vICMS   := FormatFloat('0.00',dmBaseDados.tblANotaFiscalValorICMS.AsFloat);
      vICMSDeson := '0.00';    //Versao 3.10  Valor de ICMS desonerado
      vBCST   := FormatFloat('0.00',dmBaseDados.tblANotaFiscalBaseST.AsFloat);  
      vST     := FormatFloat('0.00',dmBaseDados.tblANotaFiscalIcmsST.AsFloat);
      ...
      vPIS    := FormatFloat('0.00',dmBaseDados.tblANotaFiscalPIS.AsFloat);      
      vCOFINS := FormatFloat('0.00',dmBaseDados.tblANotaFiscalCOFINS.AsFloat);
      ...
      vTTotTrib := FormatFloat('0.00',dmBaseDados.tblANotaFiscalVTributos.AsFloat);
      if (strTotalST = 0) then
       begin
        vNF := FormatFloat('0.00',(dmBaseDados.tblANotaFiscalValorTNF.AsFloat + strDespAC + strFrete) - (strDescNFe + strDescRamo6 + strDescRamo7));
       end
      else
       begin
        vNF := FormatFloat('0.00',(dmBaseDados.tblANotaFiscalValorTNF.AsFloat + dmBaseDados.tblANotaFiscalIcmsST.AsFloat + dmBaseDados.tblANotaFiscalIPI.AsFloat + strDespAC + strFrete) - (strDescNFe + strDescRamo6 + strDescRamo7));
       end;                                                              
      Writeln(ArquivoNFe, 'W'  +'|');
      //strMsgTrib := 'Valor Aprox. Tributos R$ ' + vTTotTrib +' ('+ FormatFloat('0.00',(dmBaseDados.tblANotaFiscalVTributos.AsFloat/dmBaseDados.tblANotaFiscalValorTNF.AsFloat)*100) +'%) Fonte: IBPT.  ';
      ...
      if( (rdgTipoOperacao.ItemIndex = 0)and(ckbPrazo.Checked = True) )then
       Begin
        ...
        Writeln(ArquivoNFe, 'Y02|'  +NumFatura+ '|' +VFatura+ '|' +DescFatura+ '|' +VLiqFatura+ '|');
        if (strVenc1 <> '') then
         begin
      ...
      Writeln(ArquivoNFe, 'Z|'  +'|'  +strMensagem1  +strMensagem2  +strMensagem3  +strMensagem4  +strMensagem5  +strMensagem6  +strMsgTrib  +edtObsComp.Text  +'|');
      ...
      if (EmailDest <> '') then
       begin
        Writeln(ArquivoNFe, 'Z04|EMAIL|' + EmailDest +'|');
       end;
      Writeln(ArquivoNFe, '');
...
NFeRTXT.CarregarArquivo('F:\NFe\' + 'D' +mskNrNotaFiscal.Text +'.txt');
...
stsRetorno := ACBrNFe1.WebServices.Retorno.NFeRetorno.ProtNFe.Items[0].cStat;
strMotivo  := ACBrNFe1.WebServices.Retorno.NFeRetorno.ProtNFe.Items[0].xMotivo;
...
if (emailDest <> '') then
  Begin
    ...
    try
      ...
      ACBrMail1.FromName            := 'PERFIL AUTO PEÇAS NFe DEVOLUCAO Nº ' + dmBaseDados.tblANotaFiscalNrNF.AsString;
      strAssunto                    := 'PERFIL AUTO PEÇAS NFe DEVOLUCAO Nº ' + dmBaseDados.tblANotaFiscalNrNF.AsString;
...
procedure TformNFeDevolucao.btnRetornarClick(Sender: TObject);
begin
Close;
end;

procedure TformNFeDevolucao.btnPrimeiroServicoClick(Sender: TObject);
begin
dmBaseDados.tblLogMensal.First;
end;

procedure TformNFeDevolucao.btnUltimoServicoClick(Sender: TObject);
begin
dmBaseDados.tblLogMensal.Last;
end;

procedure TformNFeDevolucao.btnGravarServicoClick(Sender: TObject);
...
procedure TformNFeDevolucao.btnExcluirServicoClick(Sender: TObject);
begin
...
procedure TformNFeDevolucao.grdProdutosColExit(Sender: TObject);
...
procedure TformNFeDevolucao.grdProdutosKeyPress(Sender: TObject; var Key: Char);
var Sttsgrid : String;
begin
Sttsgrid := '0';
...
procedure TformNFeDevolucao.TISButton1Click(Sender: TObject);
begin
formSeekFornecedor2.ShowModal;
end;

procedure TformNFeDevolucao.edtDespAcKeyPress(Sender: TObject; var Key: Char);
begin
if not (Key in['0'..'9',Chr(8),DecimalSeparator]) then Key := #0;
end;

procedure TformNFeDevolucao.edtBaseIcmsKeyPress(Sender: TObject; var Key: Char);
begin
if not (Key in['0'..'9',Chr(8),DecimalSeparator]) then Key := #0;
end;

procedure TformNFeDevolucao.edtVIcmsKeyPress(Sender: TObject; var Key: Char);
begin
if not (Key in['0'..'9',Chr(8),DecimalSeparator]) then Key := #0;
end;

procedure TformNFeDevolucao.edtBaseSTKeyPress(Sender: TObject; var Key: Char);
begin
if not (Key in['0'..'9',Chr(8),DecimalSeparator]) then Key := #0;
end;

procedure TformNFeDevolucao.edtValorSTKeyPress(Sender: TObject; var Key: Char);
begin
if not (Key in['0'..'9',Chr(8),DecimalSeparator]) then Key := #0;
end;

procedure TformNFeDevolucao.edtTotalProdKeyPress(Sender: TObject; var Key: Char);
begin
if not (Key in['0'..'9',Chr(8),DecimalSeparator]) then Key := #0;
end;

procedure TformNFeDevolucao.edtValorIPIKeyPress(Sender: TObject; var Key: Char);
begin
if not (Key in['0'..'9',Chr(8),DecimalSeparator]) then Key := #0;
end;

procedure TformNFeDevolucao.edtVTotalNFKeyPress(Sender: TObject; var Key: Char);
begin
if not (Key in['0'..'9',Chr(8),DecimalSeparator]) then Key := #0;
end;

procedure TformNFeDevolucao.edtDespAcExit(Sender: TObject);
...
procedure TformNFeDevolucao.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TformNFeDevolucao.rdgTransClick(Sender: TObject);
begin
formDigitaTransp.ShowModal;
end;

procedure TformNFeDevolucao.mskNrNotaFiscalKeyPress(Sender: TObject; var Key: Char);
begin
